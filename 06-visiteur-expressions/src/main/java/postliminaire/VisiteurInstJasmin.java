...
public class VisiteurInstJasmin extends VisiteurInstruction<Integer> {
  ...
  public Integer visite(Affectation a) {
    ...
    code.add("istore");
    code.add(code.varIndex(a.v().nom()));
    return h;
  }
  ...
  public Integer visite(Pour pour) {
    ...
    return new Sequence(pour.init(), new TantQue(pour.cond(), new Sequence(pour.i1(), pour.inc())))
        .accepter(this);
  }

  public Integer visite(Afficher a) {
    code.add("getstatic");
    code.add("java/lang/System/out Ljava/io/PrintStream;");
    int h = a.exp().accepter(this.vej);
    code.add("invokevirtual");
    code.add("java/io/PrintStream/println(I)V");
    return h;
  }
  ...
  public Integer visite(Assertion a) {
    ...
    code.add("labelxxxxx");
    code.add("new");
    code.add("java/lang/AssertionError");
    code.add("dup");
    h++;
    if (a.message() != null) {
      code.add("ldc");
  ...
  /** appel d'une méthode écrite en Java uniquement des paramètres entier (IIII...)V */
  public Integer visite(AppelJava a) {
    int h = 1;
    for (Expression e : a.exp()) {
      h = Math.max(h, e.accepter(this.vej));
    }
    code.add("invokestatic");
    code.add(a.nom());
    return h;
  }
